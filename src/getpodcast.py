...
import mimetypes, re, shutil, os, datetime, argparse, collections, socket, sys

mimetypes.init()

# options tuple
Options = collections.namedtuple('Options',
  ['run', 'dryrun', 'onlynew', 'quiet', 'list', 'keywords', 'podcast', 'date_from', 'root_dir', 'template', 'user_agent', 'hooks'])
...
def options(**kwargs) -> Options:
  ...
  parser.add_argument('--keywords',    action='store_true', help='List available keywords for filename template')
  parser.add_argument('--template',    default=kwargs.get('template', default_template), help='Filename template')
  ...
  parser.add_argument('--user-agent',  default=kwargs.get('user_agent' , ''),            help='web request identification')
  parser.add_argument('--hooks',       default=kwargs.get('hooks' , ''),                 help='comma seperated key=value list of hooks')
...
def process_podcast_item(pod:str, item, headers: dict, fromdate, args: Options):
  ...
  if os.path.isfile(newfilename):
    ...
    try:
      if validateFile(newfilename, item.time_published, item.enclosure_length, item.enclosure_url, headers):
  ...
  cancel_validate, newfilelength = try_download_item(newfilelength, newfilename, item, headers)
  ...
  try:
    if validateFile(newfilename, 0, item.enclosure_length, item.enclosure_url, headers):
...
def validateFile(newfilename: str, time_published: int, enclosure_length: int, enclosure_url: str, headers: dict) -> bool:
